...
class FuncUv(object):
    ...
    def _func_all_year_uv(self):  #儒略日换算
    ...
    def _get_uv_coordination(self, mat_uv, pos_sta1, pos_sta2):   #未跑到该函数
    ...
    def _calculate_beam_size(self):     #未跑到该函数1
...
class UVConfigParser(object):
    ...
    def rewrite_config(self):
        ...
        config.set("obs_time", "start", "2023/01/01/00/00/00")
        config.set("obs_time", "end", "2023/01/02/00/00/00")
        config.set("obs_time", "step", "00/00/05/00")
        self.time_start = [2023, 1, 1, 0, 0, 0]
        self.time_end = [2023, 1, 2, 0, 0, 0]
        ...
        #config.set("station", "pos_source", "CasA")
        #config.set("station", "pos_vlbi", "ShangHai, Tianma, Urumqi, GIFU11, HITACHI,KASHIM34")
        config.set("station", "pos_vlbi", "GZNU1, GZNU2, GZNU3, GZNU4")
        ...
        self.str_vlbi = ['GZNU1', 'GZNU2', 'GZNU3', 'GZNU4']
        self.str_telemetry = ['']
        self.str_sat = ['']
        self.get_data_from_db()
        # write file
        ...
        if len(self.str_source) != 0:
            for each in self.str_source:
                if each in db_src_dict.keys():
                    self.pos_mat_src.append(list(db_src_dict[each]))   #观测源的信息
            #print(self.pos_mat_src)
        # sat
        self.pos_mat_sat = []
        if len(self.str_sat) != 0:
            for each in self.str_sat:
                if each in db_sat_dict.keys():
                    self.pos_mat_sat.append(list(db_sat_dict[each]))     #空
                #print(self.pos_mat_sat)
        ...
        if len(self.str_telemetry) != 0:
            for each in self.str_telemetry:
                if each in db_telem_dict.keys():
                    self.pos_mat_telemetry.append(list(db_telem_dict[each]))      #空
                #print(self.pos_mat_telemetry)
        # vlbi
        self.pos_mat_vlbi = []
        if len(self.str_vlbi) != 0:
            for each in self.str_vlbi:
                if each in db_vlbi_all.keys():
                    self.pos_mat_vlbi.append(list(db_vlbi_all[each]))       #站台信息
                #print(self.pos_mat_vlbi)
...
def run_uv_basic(ax):
    ...
    start_time = ut.time_2_mjd(*my_config_parser.time_start, 0)#儒略日算法得出
    ...
    # np.savetxt(r"C:\Users\Administrator\Desktop\optimize7-25\uv filing\testu.dat", x, fmt='%.10f')
    # np.savetxt(r"C:\Users\Administrator\Desktop\optimize7-25\uv filing\testv.dat", y, fmt='%.10f')

    # create the imgs
    fig = plt.figure(figsize=(8, 8))
    #ax = plt.subplot(111, aspect='equal')
    if x is not None and y is not None:
        x = np.array(x)# # /1000.0  单位：m
        y = np.array(y)#  # /1000.0
        ...
        # ax.set_ylim(-3,3)
        # plt.legend()
        # plt.savefig(r"C:\Users\Administrator\Desktop\optimize20240401\uv filing\mc10.png", dpi=300,format='png')
        # plt.show()
    # save uv data
    if args.save_uv != '':
    ...
    if args.img_info:
        ...
        print('e_bpa={} degree, e_bmin = {} mas, e_bmaj = {} mas, max_baseline = {}'.format(e_bpa, e_bmin, e_bmaj,
                                                                                            max_bl))
    # save uv img
    img_type = 'pdf'
    ...
    if args.img_name == "bytime":
        ...
        name = "uv-" + img_type
        uv_path = os.path.join(os.path.join(os.getcwd(), 'OUTPUT'), 'uv_basic')
        #uv_path ='./OUTPUT/uv_basic/%s' % name
        uv_path = os.path.join(uv_path, name)
        #plt.savefig(uv_path)
    else:
        name = args.img_name + '.' + img_type
        uv_path = os.path.join(os.path.join(os.getcwd(), 'OUTPUT'), 'uv_basic')
        uv_path ='./OUTPUT/uv_basic'
        uv_path = os.path.join(uv_path, name)
        plt.savefig(uv_path)
    # show uv img
